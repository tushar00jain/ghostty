...
class TerminalController: BaseTerminalController {
    ...
    static var preferredParent: TerminalController? {
        all.first { $0.isSelectedTerminal && ($0.window?.isMainWindow ?? false) }
            ?? lastMain.flatMap { $0.windowHost == nil ? nil : $0 } ?? all.last
    }
    ...
    static func newTab(
        ...
        guard let parent,
              let parentController = parent.terminalContentController as? TerminalController,
              let tabGroup = parentController.windowHost else {
            return newWindow(ghostty, withBaseConfig: baseConfig, withParent: parent)
        }

        // Create a terminal and add it to the parent's inner tab group.
        let parentIndex = tabGroup.controllers.firstIndex(of: parentController) ?? 0
        ...
        default:
            tabGroup.move(controller, to: parentIndex + 1)
        }
    ...
    func relabelTabs() { windowHost?.refreshTabs() }
    ...
    private func closeOtherTabsImmediately() {
        ...
        for controller in tabGroup.terminals where controller !== self {
    ...
    private func closeTabsOnTheRightImmediately() {
        guard let tabGroup = windowHost else { return }
        guard let currentIndex = tabGroup.terminals.firstIndex(of: self) else { return }

        let tabsToClose = tabGroup.terminals.enumerated().filter { $0.offset > currentIndex }
        ...
        for (_, controller) in tabsToClose {
            controller.closeTabImmediately(registerRedo: false)
        }
    ...
    func closeWindowImmediately() {
        ...
        windowHost?.terminals.forEach { $0.cancelPendingInitialPresentation() }
        window.close()
    }
    ...
    private func registerUndoForCloseWindow() {
        ...
        let undoStates = tabGroup.terminals
            .compactMap { controller -> UndoState? in
        ...
        if let keyController = tabGroup.selected as? TerminalController,
        ...
        if let window {
            ...
            if let tabGroup,
               let tabIndex = undoState.tabIndex {
                tabGroup.move(self, to: tabIndex)
    ...
    @IBAction func closeOtherTabs(_ sender: Any?) {
        ...
        guard tabGroup.terminals.contains(where: { controller in
            // Ignore ourself
            if controller === self { return false }
    ...
    @IBAction func closeTabsOnTheRight(_ sender: Any?) {
        ...
        let needsConfirm = tabsToClose.contains { (_, controller) in
            return controller.surfaceTree.contains(where: { $0.needsConfirmQuit })
        }
    ...
    @IBAction override func closeWindow(_ sender: Any?) {
        ...
        let confirmControllers = (windowHost?.terminals ?? [self])
    ...
    @objc private func onMoveTab(notification: SwiftUI.Notification) {
        ...
        guard isSelectedTerminal else { return }
        ...
        tabGroup.move(selectedWindow, to: finalIndex)
    }
