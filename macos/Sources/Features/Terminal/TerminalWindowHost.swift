...
final class TerminalWindowHost: NSWindowController, NSWindowDelegate, FullscreenDelegate {
    ...
    func select(_ controller: BaseTerminalController) {
        ...
        if selected !== controller {
            selected?.commandPaletteIsShowing = false
    ...
    func refreshTabs() {
        ...
        tabStrip?.update(tabs: controllers.map { .init(id: $0.tabID, title: $0.tabTitle) },
                         selectedID: selected?.tabID)
    }

    override func newWindowForTab(_ sender: Any?) {
        (selected as? TerminalController)?.newWindow(sender)
    }
    ...
    func windowShouldClose(_ sender: NSWindow) -> Bool {
        if let terminal = selected as? TerminalController {
            terminal.closeWindow(nil)
            return false
        }
        return selected?.windowShouldClose(sender) ?? true
    }
    ...
    func window(_ window: NSWindow, willEncodeRestorableState state: NSCoder) {
        guard selected is TerminalController else { return }
        TerminalWindowRestorableState(from: self).encode(with: state)
    }
