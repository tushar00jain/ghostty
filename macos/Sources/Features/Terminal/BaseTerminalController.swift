...
class BaseTerminalController: NSViewController,
    ...
    override func loadView() {
        view = TerminalViewContainer {
            TerminalView(ghostty: ghostty, viewModel: TerminalViewReference(self), delegate: self)
        }
    }
    ...
    func focusedSurfaceDidChange(to: Ghostty.SurfaceView?) {
        // Clicking window controls must not discard the terminal's remembered split.
        guard let to, surfaceTree.contains(to) else {
    ...
    var tabTitle: String { titleOverride.map { computeTitle(title: $0, bell: bell) } ?? lastComputedTitle }

    private func applyTitleToWindow() {
        windowHost?.refreshTabs()
