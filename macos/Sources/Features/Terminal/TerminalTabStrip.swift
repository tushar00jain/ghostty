...
final class TerminalTabStrip: NSView, NSCollectionViewDataSource, NSCollectionViewDelegate {
    ...
    override func layout() {
        ...
        let itemSize = NSSize(width: itemWidth, height: 28)
        if flowLayout.itemSize != itemSize { flowLayout.itemSize = itemSize }
        let documentSize = NSSize(
            width: max(scrollView.contentSize.width, CGFloat(tabs.count) * itemWidth + 20),
            height: scrollView.contentSize.height)
        if collectionView.frame.size != documentSize { collectionView.setFrameSize(documentSize) }
    ...
    func update(tabs: [Item], selectedID: UUID?) {
        ...
        let itemsChanged = self.tabs.map(\.id) != tabs.map(\.id)
    ...
    func collectionView(_ collectionView: NSCollectionView,
        ...
        tabView.update(tab, selected: tab.id == selectedID)
...
private final class TerminalTabKeyEquivalentLabel: NSTextField {
...
private final class TerminalTabItemView: NSView {
    ...
    private let keyEquivalentLabel = TerminalTabKeyEquivalentLabel(labelWithString: "")
    fileprivate private(set) var active = false
    var showsSeparator = false { didSet { if oldValue != showsSeparator { needsDisplay = true } } }
    private var trackingArea: NSTrackingArea?
    private var paintedTab: TerminalTabStrip.Item?
    var onSelect: (() -> Void)?
    var onClose: (() -> Void)?
    override init(frame frameRect: NSRect) {
        super.init(frame: frameRect)
        ...
        keyEquivalentLabel.font = .systemFont(ofSize: NSFont.smallSystemFontSize)
        keyEquivalentLabel.textColor = .secondaryLabelColor
        keyEquivalentLabel.alignment = .right
        keyEquivalentLabel.setContentHuggingPriority(.required, for: .horizontal)
        keyEquivalentLabel.setContentCompressionResistancePriority(.required, for: .horizontal)
        for child in [selectButton, closeButton, keyEquivalentLabel] {
            child.translatesAutoresizingMaskIntoConstraints = false
            addSubview(child)
        }
        NSLayoutConstraint.activate([
            selectButton.leadingAnchor.constraint(equalTo: leadingAnchor, constant: 24),
            selectButton.trailingAnchor.constraint(equalTo: keyEquivalentLabel.leadingAnchor, constant: -8),
            keyEquivalentLabel.trailingAnchor.constraint(equalTo: trailingAnchor, constant: -6),
            keyEquivalentLabel.centerYAnchor.constraint(equalTo: centerYAnchor),
            keyEquivalentLabel.widthAnchor.constraint(greaterThanOrEqualToConstant: 10),
    ...
    func update(_ tab: TerminalTabStrip.Item, selected: Bool) {
        guard paintedTab != tab || active != selected else { return }
        paintedTab = tab
        let title = tab.title
        active = selected
        needsDisplay = true
        selectButton.title = title
        keyEquivalentLabel.stringValue = tab.keyEquivalent
...
private final class TerminalTabCollectionView: NSCollectionView {
    ...
    var railRect = NSRect.zero { didSet { if oldValue != railRect { needsDisplay = true } } }
    override func draw(_ dirtyRect: NSRect) {
        super.draw(dirtyRect)
